...
class Builder:
    ...
    def scan(self, source):
        ...
        fullName = os.path.join(self.rootDir, source) 
        ...
        try:
            ...
            while (nodeStack):
               ...
               while keepReading:
                   ...
                   if m:
                       path = os.path.normpath(os.path.join(self.rootDir, m.group(1).strip()))
                   ...
                   if m and not m.group(2):
                       ...
                       for path in includePaths:
                           if os.path.exists(os.path.join(path, file)):
                               header = getStandardName(self.rootDir, os.path.join(path, file))
                               break
                       if not header:
                           continue
                       ...
                       if not headerNode.scanned:
                           ...
                           currentNode.file = open(os.path.join(self.rootDir, header))
    ...
    def callFunction(self, function, args, symbolTable, fileName, lineNumber):
        ...
        if function == "join":
            ...
            results = [] 
            for path in paths[1:]:
                results.append(os.path.join(paths[0], path))
            return " ".join(results)
        if function == "remove":
        ...
        if function == "exist":
            args = self.split(args)
            for arg in args:
                if not os.path.exists(os.path.join(self.rootDir, arg)):
                    return "0"
            return "1"
    ...
    def addNode(self, name, targets, isFile, fileName, lineNumber):
        ...
        if targets: 
            for target in targets.split():
                targetName = os.path.join(self.rootDir, target)
...
def getConfigFileName(rootDir):
    return os.path.join(rootDir, ".qi-%s-%s.conf" % (platform.node(), getpass.getuser()))
...
http://code.google.com/p/qi-make"""
    ...
    if args and args[0] == "init":
        if os.path.exists(os.path.join(workDir, PROJECT_FILE)):
            print "Error: the default project file '%s' already exists." % PROJECT_FILE
            return 1
        try: 
            projectFile = open(os.path.join(workDir, PROJECT_FILE), "w")
     ...
     # Set 'rootDir'
    rootDir = workDir 
    while not os.path.exists(os.path.join(rootDir, PROJECT_FILE)):
    ...
    try:
        ...
        projectFile = open(os.path.join(rootDir, PROJECT_FILE), "r")
        lineNumber = 0 
        currentSection = ""
        ...
        for lineNumber, line in projectSections[SOURCE_FILES_SECTION]:
            files = line.split()
            if files:
                sourceHeaderMap[files[0]] = files[1:]
    ...
    if args and (args[0] == "add" or args[0] == "delete"):
        ...
        try:
            projectFile = open(os.path.join(rootDir, PROJECT_FILE), "w") 
